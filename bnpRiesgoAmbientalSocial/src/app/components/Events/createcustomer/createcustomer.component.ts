...
export class CreatecustomerComponent implements OnInit {
  ...
  validateCustomerNumber(customerNumber: string) {
    this.uApi.callGetExecute('2',customerNumber).subscribe((res) => {
  ...
  postCustomerData() {
    ...
    this.uApi.callPostExecute(data).subscribe((res) => {
      this.dialogRef.close({ event: 'Done' });
    });
  }

  getBancaData() {
    this.uApi.callGetExecute('29', '').subscribe((res) => {
      this.arrayBanca = res;
    });
  }

  setBancaData(data: any) {
    this.userGeneralData.controls['customerDepartment'].setValue(data);
  }
