...
export class SubregistrosComponent implements OnInit {
  ...
  callSubTickets() {
    this.uApi
      .callGetExecute('6',this.data.currentUniqueKey)
      .subscribe((res) => {
        this.arrayData = res;
        this.datatable = true;
  ...
  doSomethingWithBotton(option: any, code: any, data: any) {
    ...
    if (data == 'CLA') {
      this.router.navigate(['/auth/clasificacion-view']);
      this.closeDialog('stay');
    } else if (data == 'GEN') {
      this.router.navigate(['/auth/general-view']);
      this.closeDialog('stay');
    } else if (data == 'SEG') {
      this.router.navigate(['/auth/SEG-view']);
      this.closeDialog('stay');
    } else {
      this.showForm(data);
    }
