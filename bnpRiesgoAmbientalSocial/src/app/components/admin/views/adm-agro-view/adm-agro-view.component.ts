...
@Component({
  selector: 'app-adm-agro-view',
  templateUrl: './adm-agro-view.component.html',
  styleUrls: ['./adm-agro-view.component.css'],
})
export class AdmAgroViewComponent implements OnInit {
  ...
  getRegistStatus() {
    this.uApi.callGetExecute('16', this.formCode || '').subscribe((res) => {
      let status = res[0];
      if (status.state == 'EVALUADO' || status.state == 'CORRECCIÓN') {
