...
export class GeneralViewComponent implements OnInit {
  ...
  turnEdition(option: any) {
    ...
    this.formularioFormGroup.controls['customerNumberCtrl'].disable();
  ...
  getSumHrCounts() {
    let sum: number =
      Number(this.formularioFormGroup.controls['rhCountAdmCtrl'].value) +
      Number(this.formularioFormGroup.controls['rhCountOperaCtrl'].value) +
      Number(this.formularioFormGroup.controls['rhCountTempCtrl'].value);
    this.formularioFormGroup.controls['rhConutTotalCtrl'].setValue(sum);
  }
  ...
  onlyHourDigits(event: { which: any; keyCode: any }): boolean {
    const charCode = event.which ? event.which : event.keyCode;
